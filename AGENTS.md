...
The system uses a **plugin architecture** with three provider interfaces. All engine code MUST interact with providers through abstract base classes only. Never import a specific provider (Suno, ElevenLabs, Codex, etc.) directly in engine or router code. Always go through the provider registry.
...
│   └── ...
├── scriptwriter/
│   ├── Codex.py      # ScriptWriterProvider implementation
│   └── ...
└── voice/
...
| DJ Script Writing | Anthropic Codex API (default provider) |
...
10. Codex scriptwriter provider implementation
