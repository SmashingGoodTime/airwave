...
content generator second. The current music buffer, DJ brain, talk show engine,
and scheduler continue to run as they do today, while new shared foundations are
...
| `ProgramItem` | A single item on the station timeline, such as a music track, DJ break, talk segment, call clip, live input, or fallback item. |
...
These records do not replace existing `Track`, `DJBreak`, `TalkSegment`, or
`PlayLog` records yet. They are an additive foundation for moving the scheduler
...
Generated music tracks, DJ breaks, and talk segments are now mirrored into the
...
track, DJ break, and talk segment generation now record `GenerationJob` rows for
...
timeline. When legacy tracks or talk segments are queued by the existing
scheduler, their matching `ProgramItem` rows move through `playing`, `played`,
or `failed` states. Queued DJ breaks are marked `playing` after the legacy
